...
export const calculateMaxVariantBatch = (
  ...
  const remainingGlobal = Math.max(0, totalBaseUnits - sumOfAllBatches);
...
export const validateVariantBatchAllocation = (
  ...
  const maxAllowed = calculateMaxVariantBatch(variantId, currentBatchSize, totalBaseUnits, allVariants);
